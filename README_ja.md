...
- 対戦相手のプレイヤー名をキャプチャし、透過PNG加工してOBSに反映
...
- 以下の名前で**画像**ソースを2つ作成してください（名前は完全一致が必要）：
  - `pokecham_auto-name`（対戦相手のプレイヤー名用）
  - `pokecham_auto-poke`（対戦相手のポケモン一覧用）
...
`config.py` を開き、ご自身の画面環境に合わせて座標を調整してください。
